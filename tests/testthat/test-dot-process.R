...
test_that(".process() handles interaction terms", {
  ...
  result <- .process(
    longitudinal_formula = v ~ x1 * x2,
    longitudinal_data = longitudinal_data,
    survival_formula = Surv(surv_time, status) ~ w1 * w2,
    survival_data = survival_data,
    id = "id",
    time = "time"
  )

  # Interaction creates 4 terms: 1, x1, x2, x1:x2
  expect_equal(ncol(result[[1]]$longitudinal$covariates), 4)
...
test_that(".process() preserves time ordering", {
  ...
  result <- .process(
    longitudinal_formula = v ~ x,
    longitudinal_data = longitudinal_data,
    survival_formula = Surv(surv_time, status) ~ 1,
    survival_data = survival_data,
    id = "id",
    time = "time"
  )
...
test_that(".process() handles polynomial terms", {
  ...
  result <- .process(
    longitudinal_formula = v ~ poly(x, 2),
    survival_formula = Surv(surv_time, status) ~ poly(z, 3),
    longitudinal_data = longitudinal_data,
    survival_data = survival_data,
    id = "id",
    time = "time"
...
test_that(".process() handles large datasets efficiently", {
  ...
  result <- .process(
    longitudinal_formula = v ~ x1 + x2,
    survival_formula = Surv(surv_time, status) ~ z,
    longitudinal_data = longitudinal_data,
    survival_data = survival_data,
    id = "id",
    time = "time"
...
test_that(".process() handles formulas with no covariates", {
  ...
  for (i in 1:3) {
    expect_equal(ncol(result[[i]]$longitudinal$covariates), 1)
...
test_that(".process() computes event rate with all censored", {
  ...
  longitudinal_data <- test_data$longitudinal
  survival_data <- test_data$survival
  survival_data$status <- 0 # All censored
...
test_that(".process() computes event rate with all events", {
  ...
  longitudinal_data <- test_data$longitudinal
  survival_data <- test_data$survival
  survival_data$status <- 1 # All events
...
test_that(".process() handles edge cases in data structure", {
  ...
  longitudinal_data_min <- test_data$longitudinal
  longitudinal_data_min$v <- c(1, 2, 3, 4) # Use specific values
  survival_data_min <- test_data$survival

  result <- .process(
    v ~ 1,
    longitudinal_data_min,
    Surv(time, status) ~ 1,
    survival_data_min,
    "id",
    "time"

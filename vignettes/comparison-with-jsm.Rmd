---
title: "Comparison with JSM Package"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Comparison with JSM Package}
...
plot(km_fit, xlab = "Time", ylab = "Survival Probability",
     main = sprintf("Event Rate: %.0f%%, Median: %.1f",
...
extract_coef <- function(jsm, tvcox) {
  ...
  data.frame(
    param = c("Association", "w1", "w2"),
    jsm_est = c(jsm$coefficients$alpha,
                jsm$coefficients$phi[c("w1", "w2")]),
    jsm_se = c(jsm_vcov["alpha:v"],
...
p_bias <- ggplot(bias_data, aes(x = param_label, y = bias, fill = method)) +
  ...
  labs(x = "Parameter", y = "Bias", color = NULL,

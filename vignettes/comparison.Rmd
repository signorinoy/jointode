...
The simulation results reveal persistent parameter biases in the joint ODE-Cox model across different sample sizes. For $n=400$, the JointODE method yields biases of $0.027$, $-0.154$, and $0.214$ for $\alpha_1$, $\alpha_2$, and $\alpha_3$ respectively, with coverage probabilities of $98.1\%$, $74.1\%$, and $55.6\%$. The poor coverage for $\alpha_2$ and $\alpha_3$ indicates systematic bias rather than random variation.

This identifiability issue arises from the ODE structure $\ddot{x}(t) = \beta_1 m(t) + \beta_2 \dot{m}(t) + Z(t)$, where $\beta_1=-1.0$, $\beta_2=-0.6$, and $Z(t) = \sum_{j=3}^{6}\beta_j z_j(t)$. Substituting into the Cox model yields $h(t) = h_0(t)\exp[(\alpha_1+\alpha_3\beta_1)m + (\alpha_2+\alpha_3\beta_2)\dot{m} + \alpha_3 Z]$. The model can only identify the composite effects $\gamma_1 = \alpha_1+\alpha_3\beta_1$ and $\gamma_2 = \alpha_2+\alpha_3\beta_2$, not the individual parameters. With $\beta_3=0$, the constant term that would provide direct information about $\alpha_3$ is absent, forcing identification to rely solely on the weaker signals from time-varying covariates.

...
where $\dot{m}_i(t)$ and $\ddot{m}_i(t)$ denote the biomarker’s velocity and acceleration, respectively, and $\boldsymbol{X}_i(t)$ denotes time-varying covariates. The parameters have the usual interpretations: $\omega_n > 0$ is the natural frequency, $\xi$ is the damping ratio, and $k$ is the steady-state gain.
...
For the longitudinal sub-model, there are several statistcs that are useful for inference, including $T = \frac{2\pi}{\omega_n}$ and the damping ratio $\xi$.

However, these are not directly estimated parameters. We can use the delta method to obtain their standard errors.

Since 
$$
\omega_n = \sqrt{-\beta_1}, \quad \xi = -\frac{\beta_2}{2\sqrt{-\beta_1}},
$$
we have
$$
...
$$
Thus, the variance-covariance matrix of $(\omega_n, \xi)$ can be approximated as
$$

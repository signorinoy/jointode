...
To make the model tractable, we approximate the unknown acceleration function $f$ using a single-index model:

$$f(m_i(t), \dot{m}_i(t), \mathbf{X}_i(t), t) \approx g(\boldsymbol{\beta}^{\top}\mathbf{Z}_i(t))$$

where:

- $\mathbf{Z}_i(t) = [m_i(t), \dot{m}_i(t), \mathbf{X}_i^{\top}(t), t]^{\top}$ is the feature vector
- $\boldsymbol{\beta}$ are the single-index coefficients (constrained: $\|\boldsymbol{\beta}\| = 1$)
- $g(u) = \boldsymbol{\gamma}^{\top} \mathbf{B}_g(u)$ is a flexible function parameterized by B-spline basis $\mathbf{B}_g(u)$ with coefficients $\boldsymbol{\gamma}$

### Likelihood

The joint likelihood for subject $i$ integrates over the random effect:

$$L_i(\boldsymbol{\psi}) = \int p(\mathbf{V}_i | b_i) \cdot p(T_i, \delta_i | b_i) \cdot p(b_i) \, db_i$$
...
$$
\hat{\boldsymbol{\theta}} = \arg\max_{\boldsymbol{\theta}} Q(\boldsymbol{\theta};\widehat{\boldsymbol{\beta}})
$$
...
The M-step of the EM algorithm requires maximization of the expected complete-data log-likelihood $Q(\boldsymbol{\psi})$ with respect to the parameter vector $\boldsymbol{\psi} = (\boldsymbol{\theta}, \boldsymbol{\beta})$, where $\boldsymbol{\theta}$ encompasses the hazard model parameters and $\boldsymbol{\beta}$ represents the single-index coefficients. To perform this optimization efficiently, gradient-based methods necessitate the computation of:

$$\nabla_{\boldsymbol{\psi}} Q = \sum_{i=1}^{n}\sum_{j=1}^{n_i} \frac{r_{ij}}{\sigma_e^2} \frac{\partial m_i(T_{ij})}{\partial \boldsymbol{\psi}} + \sum_{i=1}^{n} \left[\delta_i \frac{\partial \log\lambda_i(T_i|0)}{\partial \boldsymbol{\psi}} - E[e^{b_i}|\mathcal{O}_i] \frac{\partial \Lambda_i(T_i|0)}{\partial \boldsymbol{\psi}}\right]$$
...
- $\frac{\partial m_i(T_{ij})}{\partial \boldsymbol{\psi}}$: trajectory sensitivities at observation times
- $\frac{\partial \log\lambda_i(T_i|0)}{\partial \boldsymbol{\psi}}$: log hazard sensitivities at event times
- $\frac{\partial \Lambda_i(T_i|0)}{\partial \boldsymbol{\psi}}$: cumulative hazard sensitivities at event times
...
The parameter vector $\boldsymbol{\psi} = (\boldsymbol{\theta}, \boldsymbol{\beta})$ consists of the hazard model parameters $\boldsymbol{\theta} = (\boldsymbol{\eta}, \boldsymbol{\phi}, \boldsymbol{\alpha}, \boldsymbol{\gamma})$ and the single-index coefficients $\boldsymbol{\beta}$. We derive the gradient with respect to each component.
...
#### Gradient with Respect to Single-Index Coefficients $\boldsymbol{\beta}$
...
**For acceleration spline coefficients ($\boldsymbol{\gamma}$):**
...
Since the sensitivities cannot be obtained analytically due to the nonlinear nature of the acceleration function $g(\cdot)$, we employ numerical methods. Two computational strategies are available:
...
Differentiating with respect to $\boldsymbol{\psi}$ and using integration by parts:
...
This choice eliminates the $\frac{\partial \mathbf{s}}{\partial \boldsymbol{\psi}}$ terms, yielding:
$$\frac{\partial \mathbf{s}_k(T)}{\partial \boldsymbol{\psi}} = \int_0^T \tilde{\boldsymbol{\kappa}}^{\top} \frac{\partial F}{\partial \boldsymbol{\psi}}\bigg|_{\mathbf{s}} dt$$
...
For the state vector $\mathbf{s} = [\Lambda_i, m_i, \dot{m}_i]^{\top}$, we need:
1. $\frac{\partial m_i(T_{ij})}{\partial \boldsymbol{\psi}}$ for each observation time $T_{ij}$
2. $\frac{\partial \Lambda_i(T_i)}{\partial \boldsymbol{\psi}}$ for the event time $T_i$

##### Adjoint System

The transposed Jacobian for the JointODE dynamics is:
$$\left(\frac{\partial F}{\partial \mathbf{s}}\right)^{\top} = \begin{bmatrix}
0 & 0 & 0 \\
\alpha_0 \lambda_i(t|0) & 0 & g'(u) \beta_1 \\
\alpha_1 \lambda_i(t|0) & 1 & g'(u) \beta_2
\end{bmatrix}$$

where $g'(u) = \boldsymbol{\gamma}^{\top}\mathbf{B}'_g(u)$ denotes the derivative of the acceleration function at $u = \boldsymbol{\beta}^{\top}\mathbf{Z}_i(t)$, and $\beta_1$ and $\beta_2$ are the first two components of the single-index coefficient vector $\boldsymbol{\beta}$ corresponding to $m_i(t)$ and $\dot{m}_i(t)$, respectively.

This yields the adjoint differential equations (solved backward in time):
$$\begin{aligned}
\frac{d\tilde{\kappa}_1}{dt} &= 0 \\
\frac{d\tilde{\kappa}_2}{dt} &= -\alpha_0 \lambda_i(t|0) \tilde{\kappa}_1 - \tilde{\kappa}_3 \\
\frac{d\tilde{\kappa}_3}{dt} &= -\alpha_1 \lambda_i(t|0) \tilde{\kappa}_1 - g'(u) \beta_1 \tilde{\kappa}_2 - g'(u) \beta_2 \tilde{\kappa}_3
\end{aligned}$$

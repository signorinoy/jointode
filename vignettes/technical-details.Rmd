...
1. $\frac{\partial m_i(T_{ij})}{\partial \boldsymbol{\theta}}$ for each observation time $T_{ij}$
2. $\frac{\partial \Lambda_i(T_i)}{\partial \boldsymbol{\theta}}$ for the event time $T_i$
...
0 & 0 & 0 \\
\alpha_0 \lambda_i(t|0) & 0 & \beta_1 \\
\alpha_1 \lambda_i(t|0) & 1 & \beta_2
\end{bmatrix}$$

where $\beta_1$ and $\beta_2$ are the first two components of the linear coefficient vector $\boldsymbol{\beta}$ corresponding to $m_i(t)$ and $\dot{m}_i(t)$, respectively.
...
\frac{d\tilde{\kappa}_3}{dt} &= -\alpha_1 \lambda_i(t|0) \tilde{\kappa}_1 - \beta_1 \tilde{\kappa}_2 - \beta_2 \tilde{\kappa}_3
\end{aligned}$$
...
The partial derivatives $\frac{\partial F}{\partial \boldsymbol{\theta}}\bigg|_{\mathbf{s}}$ needed for the sensitivity integrals:

**Trajectory parameters:**

- Linear acceleration coefficients: $\frac{\partial F}{\partial \boldsymbol{\beta}} = [0, 0, \mathbf{Z}_i(t)]^{\top}$
...
The JointODE model includes the biomarker trajectory and its derivatives in the hazard function:

$$\lambda_i(t) = \lambda_0(t) \exp\left[\alpha_0 m_i(t) + \alpha_1 \dot{m}_i(t) + \alpha_2 \ddot{m}_i(t) + \mathbf{W}_i^{\top}\boldsymbol{\phi} + b_i\right]$$

Since $\dot{m}_i(t)$ and $\ddot{m}_i(t)$ are derivatives of $m_i(t)$, these components are correlated, leading to identifiability challenges. The velocity effect is particularly difficult to identify. Additionally, flexible baseline hazards can absorb trajectory effects, creating a trade-off between model flexibility and parameter identifiability.
